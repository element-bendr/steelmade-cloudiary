...
import { 
  ROUTE_PARAMS, 
  extractSeriesParams, // Corrected import
  buildSeriesPath,
  buildCategoryPath
...
interface NestedSeriesPageProps {
  params: {
    ...
    // Removed ROUTE_PARAMS.SERIES1, as it does not exist
    nestedSeriesId: string; // Use a custom param for nested series
  };
}
            ...
            <li>
              <div className="flex items-center">
                ...
                <Link href={buildSeriesPath(categoryId, seriesId)} className="text-gray-700 hover:text-blue-600">
                  {seriesId.split('-').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                </Link>
              </div>
            </li>
            <li aria-current="page">
              <div className="flex items-center">
                ...
                <span className="text-gray-500">
                  {nestedSeriesId.split('-').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}
                </span>
              </div>
            </li>
          </ol>
