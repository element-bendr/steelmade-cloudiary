...
export async function generateMetadata(
  ...
  const { category, collectionId } = await params;
  ...
  const series = await getSeriesById(category, collectionId);
  ...
  return {
    title: `${series.title} Collection | SteelMade ${category}`,
...
export default async function CollectionPage({ params }: CollectionPageProps) {
  ...
  const products = await getProductsByCategoryAndSeries(category, collectionId);

  // Get related collections from same category
  const allSeries = await getAllSeries(category);
  ...
  const otherCategories = ["chairs", "tables", "accessories", "desks", "storage", "lighting"]
    .filter(cat => cat !== category && isValidCategorySlug(cat)) as ProductCategorySlug[];
  ...
  return (
    <>
      <CollectionNav currentCategory={category} />
